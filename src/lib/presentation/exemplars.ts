...
export const EXEMPLARS: Exemplar[] = [
  ...
  },
  {
    ...
    title: "Nights and weather",
    // Every asset below was opened and described from what is actually in the
    // file. Alt text that invents a scene is worse than none.
    body: `Four frames from walks I took instead of writing. The gallery keeps the set together and lets the pictures lead.`,
    fields: {},
    assets: [
      {
        id: "night-fence",
        kind: "image",
        src: "/covers/cover-118.jpg",
        alt: "The Milky Way over a leaning wooden fence at night",
        width: 1600,
        height: 900,
      },
      {
        id: "valley-mist",
        kind: "image",
        src: "/covers/cover-119.jpg",
        alt: "Light falling through mist into a forested mountain valley",
        width: 1600,
        height: 900,
      },
      {
        id: "river-bridge",
        kind: "image",
        src: "/covers/cover-120.jpg",
        alt: "A domed cathedral and a footbridge lit above a river at night",
        width: 1600,
        height: 900,
      },
      {
        id: "rain-beads",
        kind: "image",
        src: "/covers/cover-121.jpg",
        alt: "Rain beaded across taut grey fabric, close up",
        width: 1600,
        height: 900,
      },
    ],
...
/** The exemplar for one template id, if one exists. Every built-in template
 * has one; the render gate enforces the templates and the showcase publishes
 * these, so a missing entry is a wiring bug, not a normal state. */
export function exemplarFor(templateId: string): Exemplar | null {
  return EXEMPLARS.find((entry) => entry.template === templateId) ?? null;
}
