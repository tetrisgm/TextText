...
.tt-document{--tt-accent:#0071e3;--tt-gap-none:0;--tt-gap-xs:.35rem;--tt-gap-sm:.75rem;--tt-gap-md:1.25rem;--tt-gap-lg:2rem;--tt-gap-xl:3.5rem;--tt-measure:46rem;color:var(--ink,#1d1d1f);background:var(--paper,#fff);min-height:100%;font-family:var(--tt-font,-apple-system,BlinkMacSystemFont,"SF Pro Text","Helvetica Neue",sans-serif)}
...
.tt-badge{display:inline-flex;flex-wrap:wrap;align-items:center;gap:.4rem}
...
.tt-facts-table{display:grid;grid-template-columns:auto 1fr;gap:.4rem 1.25rem;margin:0}
...
.tt-checklist-item[data-done] .tt-checklist-label{color:var(--muted,#6e6e73);text-decoration:line-through;text-decoration-color:color-mix(in srgb,var(--ink,#1d1d1f) 30%,transparent)}
.tt-rows{width:100%}
...
.tt-rows-steps{list-style:none;margin:0;padding:0;counter-reset:tt-step;display:flex;flex-direction:column;gap:1rem}
...
.tt-rows-timeline{list-style:none;margin:0;padding:0 0 0 1.1rem;border-left:2px solid color-mix(in srgb,var(--ink,#1d1d1f) 14%,transparent);display:flex;flex-direction:column;gap:1rem}
...
.tt-callout{--tt-tone:var(--tt-tone-neutral);margin:0;padding:1rem 1.15rem;border-radius:.6rem;background:color-mix(in srgb,var(--tt-tone) 9%,transparent);border-left:3px solid color-mix(in srgb,var(--tt-tone) 55%,transparent);display:flex;flex-direction:column;gap:.5rem}
...
.tt-quote{margin:0;overflow-wrap:anywhere}
.tt-quote p{margin:0}
...
.tt-document[data-template="texttext.gallery"]>.tt-stack{min-height:100vh}.tt-document[data-template="texttext.gallery"] [data-tt-node="gallery-copy"],.tt-document[data-template="texttext.gallery"] [data-tt-node="gallery-media"]{width:50%;max-height:100vh;overflow:auto;padding:clamp(1rem,4vw,4rem)}
