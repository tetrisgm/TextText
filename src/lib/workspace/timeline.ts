...
export function reconcileTimeline(previous: TimelinePage, fresh: TimelinePage) {
  ...
  const entries = previous.entries.flatMap((entry) => {
    ...
    return covered(entry) ? [] : [entry];
  });
  const hasArrivals = fresh.entries.some((entry) => !previous.entries.some((old) => old.id === entry.id && old.at === entry.at));
  return {
    visible: { ...previous, entries, nextCursor: fresh.nextCursor ? previous.nextCursor : null },
