...
export type ArtifactPane = "home" | "headlines" | "profile";

export function artifactPaneFromSearch(search: string): ArtifactPane {
  const pane = new URLSearchParams(search).get("pane");
  return pane === "headlines" || pane === "profile" ? pane : "home";
}

export function ArtifactIcon({ name }: { name: "home" | "headlines" | "profile" | "search" | "bell" | "bookmark" | "history" | "settings" | "folder" }) {
...
export function ArtifactNavigation({ pane, onSelect }: { pane: ArtifactPane; onSelect: (pane: ArtifactPane) => void }) {
  return <nav className="artifact-navigation" aria-label="Main navigation">
    {(["home", "headlines", "profile"] as const).map((target) => <button
      key={target}
      type="button"
      aria-label={target === "home" ? "Home" : target === "headlines" ? "Headlines" : "Profile"}
