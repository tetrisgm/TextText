...
/// plain files. TextText-created documents keep assets inside their TextBundle;
...
public enum TextTextCentralAttachments {
    public static func dataContainerItem() -> TextTextItem {
        folder(
            identifier: .dataContainer, parent: .rootContainer,
            filename: "Data")
    }

    public static func attachmentsContainerItem() -> TextTextItem {
        folder(
            identifier: .attachmentsContainer, parent: .dataContainer,
            filename: "Attachments")
    }

    public static func workspaceItem(handle: String, displayName: String) -> TextTextItem {
        folder(
    ...
    public static func workspaceFolderFilename(
        ...
        TextTextFilename.collisionComponent(
            displayName.isEmpty ? handle : displayName, stableId: handle)
    }

    public static func documentItem(for item: TextTextItem) -> TextTextItem? {
    ...
    public static func assetItem(
        _ artifact: TextTextArtifact, handle: String, postId: String,
        size: Int? = nil
    ) -> TextTextItem {
        TextTextItem(
            ...
            serverId: nil,
            contentHash: TextTextStableDigest.sha256Hex(artifact.url),
            documentSize: size, creationDate: nil, contentModificationDate: nil,
            capabilities: .readOnlyFile, manifestURL: artifact.url)
    }

    public static func documentFolderFilename(for item: TextTextItem) -> String {
        ...
        return TextTextFilename.collisionComponent(
            TextTextFilename.decodeComponent(stem), stableId: serverId)
    }
    ...
    private static func folder(
        identifier: TextTextItemIdentifier, parent: TextTextItemIdentifier,
        filename: String
    ) -> TextTextItem {
        TextTextItem(
