...
// The wire shapes the TextText platform's /api/sync/v1 routes emit, mirrored here
...
public enum TextTextFileRepresentation: String, Codable, CaseIterable, Sendable {
    case textbundle
    case markdown
    case text
    ...
    /// a package (see TextTextFileProviderItem.contentType).
    case textpack
    ...
    public static func inferred(fromFilename filename: String) -> TextTextFileRepresentation? {
...
public struct TextTextWorkspaceBlog: Codable, Equatable, Sendable {
...
public struct TextTextWorkspaceFolder: Codable, Equatable, Sendable {
...
public struct TextTextWorkspace: Codable, Equatable, Sendable {
    public let blog: TextTextWorkspaceBlog
    public let folders: [TextTextWorkspaceFolder]

    public init(blog: TextTextWorkspaceBlog, folders: [TextTextWorkspaceFolder]) {
        self.blog = blog
...
public struct TextTextManifestItem: Codable, Equatable, Sendable {
    ...
    public init(
        file: String, representation: TextTextFileRepresentation = .markdown,
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.init(
            ...
            representation: try values.decodeIfPresent(
                TextTextFileRepresentation.self, forKey: .representation) ?? .markdown,
    ...
    public func contentHash(for representation: TextTextFileRepresentation? = nil) -> String {
        let form = representation ?? self.representation
        return form.isTextBundleFamily ? documentHash ?? hash : hash
    }

    public func contentSize(for representation: TextTextFileRepresentation? = nil) -> Int? {
        let form = representation ?? self.representation
        return form.isTextBundleFamily ? documentSize ?? size : size
    }
}

/// One immutable binary referenced by a TextText document. The server assigns a
/// deterministic, portable filename; File Provider validates both the name and
/// the TextText-owned URL before materializing it in a package or attachment tree.
public struct TextTextArtifact: Codable, Equatable, Sendable {
...
public struct TextTextArtifactManifest: Codable, Equatable, Sendable {
    public let postId: String
    public let slug: String
    public let fileHash: String
    public let documentHash: String?
    public let artifacts: [TextTextArtifact]
    ...
    public func contentHash(for representation: TextTextFileRepresentation) -> String {
        representation.isTextBundleFamily ? documentHash ?? fileHash : fileHash
    }
}

public struct TextTextArtifactContent: Equatable, Sendable {
...
/// only by inequality.
public struct TextTextChangeReply: Codable, Equatable, Sendable {
