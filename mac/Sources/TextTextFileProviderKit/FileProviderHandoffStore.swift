...
private let handoffLog = Logger(subsystem: "app.texttext", category: "fileprovider-handoff")
...
/// (`<TeamID>.app.texttext.fp`) is stamped into each bundle's Info.plist at
/// build time as `TextTextKeychainAccessGroup`.
public enum FileProviderHandoffStore {
    private static let service = "app.texttext.fileprovider"
    ...
    public static func accessGroup() -> String? {
        if let env = ProcessInfo.processInfo.environment["TEXTTEXT_KEYCHAIN_GROUP"],
           !env.isEmpty {
            return env
        }
        guard let group = Bundle.main
            .object(forInfoDictionaryKey: "TextTextKeychainAccessGroup") as? String,
              !group.isEmpty, group != "TEXTTEXT_KEYCHAIN_GROUP" else {
            return nil
        }
        return group
    }
    ...
    /// TextText (or replace) the handoff. Returns false if there is no access group
