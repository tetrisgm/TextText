...
public struct WorkspaceEnumerator: Sendable {
    private let api: TextTextSyncAPI
    ...
    public init(
        api: TextTextSyncAPI, handle: String, workspaceName: String,
    ...
    public func rootItem() -> TextTextItem {
    ...
    public func workspaceItem() -> TextTextItem {
        TextTextItemMapper.workspaceItem(handle: handle, name: workspaceName, readOnly: readOnly)
    }
    ...
    public func children(
        of container: TextTextItemIdentifier
    ...
    public func item(
        for identifier: TextTextItemIdentifier
    ) async -> Result<TextTextItem, TextTextSyncError> {
        switch identifier {
        case .rootContainer:
        ...
        case .folder(_, let id):
            ...
            case .success(let ws):
                ...
                let siblings: Result<[TextTextItem], TextTextSyncError>
    ...
    public func currentCursor() async -> Result<String, TextTextSyncError> {
    ...
    public func containerAnchor(
        for container: TextTextItemIdentifier
    ) async -> Result<Data, TextTextSyncError> {
    ...
    public static func fingerprint(_ items: [TextTextItem]) -> Data {
        var canonical = Data()
        ...
        return TextTextStableDigest.sha256(canonical)
    }
    ...
    static func canonicalEncoding(_ item: TextTextItem) -> Data {
        var canonical = Data()
        let fields = [
    ...
    public static func itemDigest(_ item: TextTextItem) -> String {
        TextTextStableDigest.sha256Hex(canonicalEncoding(item))
    }

    // MARK: - internals

    private func topLevelFolders() async -> Result<[TextTextItem], TextTextSyncError> {
        switch await api.workspace() {
        case .failure(let e): return .failure(e)
        case .success(let ws):
            ...
            return .success(TextTextFilename.disambiguate(items))
        }
    }
    ...
    private func folderChildren(
        ...
        case .success(let entries):
            let files = entries.compactMap {
                TextTextItemMapper.item(for: $0, inFolder: folderId, handle: handle, readOnly: readOnly)
            }
            // Files and subfolders occupy the same Finder namespace.
            return .success(TextTextFilename.disambiguate(subfolders + files))
        }
    }

    private func everything() async -> Result<[TextTextItem], TextTextSyncError> {
        let ws: TextTextWorkspace
        switch await api.workspace() {
        case .failure(let e): return .failure(e)
        case .success(let value): ws = value
        ...
        let folderItems = ws.folders.map {
            TextTextItemMapper.item(for: $0, handle: handle, readOnly: readOnly)
        }
        ...
        var indexById: [TextTextItemIdentifier: Int] = [:]
        var files: [TextTextItem] = []
        for folder in ws.folders {
            switch await api.manifest(folderId: folder.id) {
            case .failure(let e): return .failure(e)
            case .success(let entries):
                for entry in entries {
                    guard let item = TextTextItemMapper.item(
