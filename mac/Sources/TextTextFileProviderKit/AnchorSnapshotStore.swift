...
public struct TextTextContainerDelta: Equatable, Sendable {
    public let updated: [TextTextItem]
    ...
    public static func compute(
        previous: [String: String], current: [TextTextItem]
    ) -> TextTextContainerDelta {
        var updated: [TextTextItem] = []
        ...
        return TextTextContainerDelta(updated: updated, deletedIdentifiers: deleted)
    }

    /// The id->digest snapshot for a child list, stored alongside the anchor it
    /// fingerprints.
    public static func snapshot(of items: [TextTextItem]) -> [String: String] {
...
public struct AnchorSnapshotStore: Sendable {
    ...
    public func save(container: TextTextItemIdentifier, anchor: Data, items: [TextTextItem]) {
        let stored = Stored(
            anchor: anchor, items: TextTextContainerDelta.snapshot(of: items))
    ...
    public func loadSnapshot(
        container: TextTextItemIdentifier, matching anchor: Data
    ) -> [String: String]? {
    ...
    private func fileURL(for container: TextTextItemIdentifier) -> URL {
        // Container raw values contain ":"; a digest filename is filesystem-safe.
        directory.appendingPathComponent(
            TextTextStableDigest.sha256Hex(container.rawValue) + ".json",
            isDirectory: false)
    }
}
