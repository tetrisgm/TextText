...
public struct CreateDocumentIntent: AppIntent {
    ...
    public func perform() async throws -> some IntentResult & ReturnsValue<WriteDocumentEntity> {
        let record = try WorkspaceIntentActions().createDocument(title: titleText, body: body, folderPath: folderPath)
        return .result(value: WriteDocumentEntity(record: record))
    }
}
...
public struct OpenDocumentIntent: AppIntent {
    ...
    public var document: WriteDocumentEntity
...
public struct SearchDocumentsIntent: AppIntent {
    ...
    public func perform() async throws -> some IntentResult & ReturnsValue<[WriteDocumentEntity]> {
        ...
        return .result(value: records.map(WriteDocumentEntity.init(record:)))
    }
}
...
public struct CreateFolderIntent: AppIntent {
    ...
    public func perform() async throws -> some IntentResult & ReturnsValue<WriteFolderEntity> {
        let folder = try WorkspaceIntentActions().createFolder(name: name, parentPath: parentPath)
        return .result(value: WriteFolderEntity(record: folder))
    }
}
...
public struct CreateBookmarkFromURLIntent: AppIntent {
    ...
    public func perform() async throws -> some IntentResult & ReturnsValue<WriteBookmarkEntity> {
        let record = try WorkspaceIntentActions().createBookmark(from: url, title: titleText)
        return .result(value: WriteBookmarkEntity(record: record))
    }
}
...
public struct PublishDocumentIntent: AppIntent {
    ...
    public func perform() async throws -> some IntentResult & ReturnsValue<WritePublicationEntity> {
        let publication = try WorkspaceIntentActions().publishDocument(id: document.id)
        return .result(value: WritePublicationEntity(record: publication))
    }
}
