...
enum PreparedInboxItem: Equatable {
    ...
    /// Append `text` to the existing server document `targetWriteId`.
    case append(targetWriteId: String, text: String)
    ...
    func prepare(_ record: InboxRecord) throws -> PreparedInboxItem {
        ...
        case .draft:
            return .create(
                ...
                idempotencyKey: idempotencyKey)
        case .append:
            guard let targetWriteId = item.targetWriteId?
                .trimmingCharacters(in: .whitespacesAndNewlines),
                  !targetWriteId.isEmpty else {
                throw InboxFilerError.missingAppendTarget
            }
            return .append(
                targetWriteId: targetWriteId, text: normalizedBody(item.text))
        case .file:
