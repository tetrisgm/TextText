    ...
    NSMenuItemValidation
{
    ...
    private let quickCaptureQueue = DispatchQueue(
        label: "app.texttext.quick-capture", qos: .userInitiated)
    private let openFileQueue = DispatchQueue(
        label: "com.example.texttext.mac.open-files", qos: .userInitiated)
    ...
    func application(_ application: NSApplication, open urls: [URL]) {
        for url in urls where url.scheme == "texttext-app" {
            openTextTextItemURL(url)
        }
        let fileURLs = urls.filter { $0.scheme != "texttext-app" }
    ...
    private func openExternalOrFileProviderItem(_ url: URL) {
        NSFileProviderManager.getIdentifierForUserVisibleFile(at: url) {
            ...
            guard let self else { return }
            let belongsToTextText = OpenFileHandler.isTextTextFileProviderItem(identifier?.rawValue)
                || domainIdentifier?.rawValue == Self.fileProviderDomainId
            if belongsToTextText {
    ...
    private func createSyncedNote(_ item: ExternalNoteImport) {
        ...
        case .success(.saved(let saved)):
            ...
            let target = TextTextItemOpenTarget(
    ...
    private func openInMainWindow(
        _ target: TextTextItemOpenTarget,
    ...
    private func enqueueQuickCapture(_ capture: QuickCaptureIntent) {
        ...
        do {
            try outbox.enqueue(
                capture,
                workspaceHandle: store.cachedWorkspace()?.blog.handle)
            refreshQuickCaptureFeedback()
            retryQuickCaptureDrain()
        } catch {
    ...
    private func drainQuickCaptureOutbox() {
        ...
        if usedCachedWorkspace
            && (summary.shouldRetry || !summary.deferredMessages.isEmpty)
        {
            switch client.workspace() {
            case .success(let (freshWorkspace, data)):
                ...
                summary.deferredMessages = refreshed.deferredMessages
        ...
        for message in summary.deferredMessages {
            appendActivity("Capture waiting: \(message)")
        }
    ...
    private func refreshQuickCaptureFeedback() {
        ...
        let receipts = handle.map {
            outbox.recentSavedReceipts(workspaceHandle: $0)
        } ?? []
        if failed > 0 {
            ...
            updateQuickCaptureFeedback(.saved(receipts))
        } else {
    ...
    @objc private func openLastQuickCaptureAction() {
        guard let handle = store.cachedWorkspace()?.blog.handle,
            let receipt = quickCaptureOutbox?.recentSavedReceipts(
                workspaceHandle: handle).first
        else { return }
        openInMainWindow(
    ...
    @objc private func undoLastQuickCaptureAction() {
        guard let handle = store.cachedWorkspace()?.blog.handle,
            let receipt = quickCaptureOutbox?.recentSavedReceipts(
                workspaceHandle: handle).first,
            receipt.canUndo
        ...
        confirmation.messageText = "Undo this capture?"
        confirmation.informativeText =
            "\(receipt.title) will be removed from \(receipt.folderPath)."
        confirmation.alertStyle = .warning
        confirmation.addButton(withTitle: "Undo capture")
        confirmation.addButton(withTitle: "Cancel")
        guard confirmation.runModal() == .alertFirstButtonReturn else { return }

        quickCaptureQueue.async { [weak self] in
            guard let self, let outbox = self.quickCaptureOutbox,
                let credentials = self.store.loadCredentials()
            else {
                DispatchQueue.main.async { [weak self] in
                    self?.presentQuickCaptureUndoError(
                        "Sign in again, then retry Undo. Nothing changed.")
                }
                return
            }
            let client = ServerClient(
            ...
            let result = QuickCaptureUndoer(outbox: outbox).undo(
                receipt, workspaceHandle: handle, client: client)
            DispatchQueue.main.async { [weak self] in
                guard let self else { return }
                switch result {
                case .success:
                    self.updateQuickCaptureFeedback(.undone(receipt.title))
                    self.appendActivity("Undid capture \(receipt.title)")
                    self.signalFileProviderChange(serverReachable: true)
                case .failure(.workspaceMismatch):
                    self.presentQuickCaptureUndoError(
                        "Switch back to \(handle), then retry Undo. Nothing changed.")
                case .failure(.unavailable):
                    self.presentQuickCaptureUndoError(
                        "This older receipt cannot be undone safely. Open the item to review it.")
                case .failure(.couldNotPersist):
                    self.presentQuickCaptureUndoError(
                        "The item was removed, but TextText could not clear its receipt. Retry Undo to reconcile it.")
                case .failure(.server):
                    self.presentQuickCaptureUndoError(
                        "The server did not confirm Undo. Nothing changed.")
                }
            }
        }
    }

    private func presentQuickCaptureUndoError(_ message: String) {
        let alert = NSAlert()
        alert.messageText = "Could not undo the capture"
    ...
    @objc private func reviewFailedCapturesAction() {
        if quickCaptureRecoveryController == nil {
            quickCaptureRecoveryController = QuickCaptureRecoveryController(
                ...
                retryOne: { [weak self] id in
                    guard let self, let outbox = self.quickCaptureOutbox else {
                        throw QuickCaptureOutboxError.couldNotPersist(
                            "The capture outbox is unavailable")
                    }
                    let restored = try outbox.retryRejectedRecord(id: id)
                    self.refreshQuickCaptureFeedback()
                    self.retryQuickCaptureDrain()
                    return restored
                },
                discardOne: { [weak self] id in
                    guard let self, let outbox = self.quickCaptureOutbox else {
                        throw QuickCaptureOutboxError.couldNotPersist(
                            "The capture outbox is unavailable")
                    }
                    let discarded = try outbox.discardRejectedRecord(id: id)
                    self.refreshQuickCaptureFeedback()
                    return discarded
                },
    ...
    private func watchForShareContainerCreation() {
        ...
        // listing it. Watch our own container when it exists; when it does not,
        // there is nothing of ours to watch yet and configureShareInbox retries
        // when the extension next runs.
        guard let groupIdentifier = Self.shareInboxGroupIdentifier(),
              let ours = Self.shareInboxCandidates(for: groupIdentifier)
    ...
    private func drainShareInbox(containerURL: URL) {
        ...
        for record in records {
            do {
                ...
                case let .append(targetTextTextId, text):
                    if appendSharedText(
                        text, toDocument: targetTextTextId, client: client,
    ...
    private static func spotlightRelativePath(
        ...
        while let value = current, visited.insert(value.id).inserted {
            chain.append(TextTextFilename.encodeComponent(value.name))
            current = value.parentId.flatMap { foldersById[$0] }
        }
        let representation = TextTextFileRepresentation.inferred(
    ...
    private func openTextTextItemURL(_ url: URL) {
        ...
        if let action {
            guard let rawTarget = query.first(where: { $0.name == "url" })?.value,
                  let target = validatedTextTextWebURL(rawTarget) else {
        ...
        openTextTextItem(link)
    }

    private func validatedTextTextWebURL(_ value: String) -> URL? {
    ...
    /// Main thread. Resolve a TextText deep link through the File Provider: the
    ...
    private func openTextTextItem(_ link: TextTextItemLink) {
    ...
    func validateMenuItem(_ menuItem: NSMenuItem) -> Bool {
        guard menuItem.action == #selector(openLastQuickCaptureAction)
            || menuItem.action == #selector(undoLastQuickCaptureAction)
        else {
            return true
        }
        guard let handle = store.cachedWorkspace()?.blog.handle else {
            return false
        }
        guard let receipt = quickCaptureOutbox?.recentSavedReceipts(
            workspaceHandle: handle).first
        else { return false }
        if menuItem.action == #selector(undoLastQuickCaptureAction) {
            return receipt.canUndo
        }
        return true
    }
    ...
    private func openTextTextItem(
        ...
        let identifier = NSFileProviderItemIdentifier(
            rawValue: TextTextItemIdentifier.file(handle: handle, id: id).rawValue)
        resolveTextTextFileProviderURL(identifier, manager: manager) { [weak self] url in
            guard let self else { return }
            guard let url else {
    ...
    private func resolveTextTextFileProviderURL(
        ...
        manager.getUserVisibleURL(for: identifier) { [weak self] url, _ in
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.7) {
                self.resolveTextTextFileProviderURL(
    ...
    private static let fileProviderDomainId = "texttext"
    ...
    private func syncFileProviderDomain(
        ...
        let handoff = FileProviderHandoff(version: 1, workspaces: [
            TextTextFileProviderKit.FileProviderWorkspace(
        ...
        if fileProviderDesiredIdentity == identity {
            guard persistFileProviderHandoff(handoff) else {
    ...
    private func removeFileProviderDomain() {
        ...
        NSFileProviderManager.getDomainsWithCompletionHandler { [weak self] domains, _ in
            DispatchQueue.main.async {
                ...
                // Sign-out removes TextText from Locations, but all downloaded files
        ...
        if let workspace = store.cachedWorkspace() {
            ...
            manager.signalEnumerator(
                for: NSFileProviderItemIdentifier(
                    rawValue: TextTextItemIdentifier.workspace(handle).rawValue)
            ) { _ in }
            for folder in workspace.folders {
                manager.signalEnumerator(
                    for: NSFileProviderItemIdentifier(
                        rawValue: TextTextItemIdentifier.folder(
    ...
    @discardableResult
    private func persistFileProviderHandoff(_ handoff: FileProviderHandoff) -> Bool {
    ...
    private func moveToApplicationsIfNeeded(interactive: Bool = false) -> Bool {
        ...
        if !interactive && ProcessInfo.processInfo.environment["TEXTTEXT_DEV_NO_MOVE"] == "1" { return false }
    ...
    func menuNeedsUpdate(_ menu: NSMenu) {
        ...
        if case .saved = quickCaptureFeedback {
            // The saved status row above opens the same exact artifact.
        } else if let handle = store.cachedWorkspace()?.blog.handle,
            let receipt = quickCaptureOutbox?.recentSavedReceipts(
                workspaceHandle: handle).first
        {
            menu.addItem(item(
                "Open \(receipt.title) in \(receipt.folderPath)",
                #selector(openLastQuickCaptureAction)))
        }
        if let handle = store.cachedWorkspace()?.blog.handle,
            let receipt = quickCaptureOutbox?.recentSavedReceipts(
                workspaceHandle: handle).first,
            receipt.canUndo
        {
            menu.addItem(item(
                "Undo capture \(receipt.title)",
                #selector(undoLastQuickCaptureAction)))
        }
        ...
        menu.addItem(item(
            "Capture clipboard", #selector(captureClipboardAction),
    ...
    /// Make TextText the system default for .md files (from the status window). No
    ...
    private func makeTextTextDefaultForMarkdown() {
    ...
    private func buildMainMenu() -> NSMenu {
        ...
        let quickCapture = file.addItem(
            ...
            keyEquivalent: " ")
        quickCapture.keyEquivalentModifierMask = [.command, .shift]
        quickCapture.target = self
        let openCapture = file.addItem(
            withTitle: "Open last capture",
            action: #selector(openLastQuickCaptureAction),
            keyEquivalent: "")
        openCapture.target = self
        let undoCapture = file.addItem(
            withTitle: "Undo last capture",
            action: #selector(undoLastQuickCaptureAction),
            keyEquivalent: "")
        undoCapture.target = self
        let failedCaptures = file.addItem(
            withTitle: "Review failed captures…",
