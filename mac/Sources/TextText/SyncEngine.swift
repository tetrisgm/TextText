...
final class SyncEngine {
    ...
    private let queue = DispatchQueue(label: "com.example.texttext.mac.sync", qos: .utility)
    ...
    var isInsideTextTextFileProviderMount: (URL) -> Bool = {
        SyncEngine.defaultIsInsideTextTextFileProviderMount($0)
    }
    ...
    private static func defaultIsInsideTextTextFileProviderMount(_ url: URL) -> Bool {
        ...
        let nameFallback = mountName == "TextText" || mountName.hasPrefix("TextText-")
            || mountName == "TextText" || mountName.hasPrefix("TextText-")
        ...
        NSFileProviderManager.getIdentifierForUserVisibleFile(at: probeURL) {
            ...
            probe.finish(with: raw.map {
                $0 == "texttext" || $0.hasPrefix("workspace-")
            })
            semaphore.signal()
        }
    ...
    private func startWatcher(root: URL, context: PassContext) {
        guard isCurrent(context) else { return }
        let isFileProviderMount = isInsideTextTextFileProviderMount(root)
        guard isCurrent(context) else { return }
    ...
    private func runPass(
        ...
        // ensureSkeleton so no `~/TextText` directory is created, cancel any
    ...
    private func performPass(
        ...
        let isFileProviderMount = isInsideTextTextFileProviderMount(context.root)
        guard isCurrent(context) else { return summary }
        guard !isFileProviderMount else {
            ...
            if fm.fileExists(atPath: target.path) {
                if source.path.compare(target.path, options: [.caseInsensitive, .literal]) == .orderedSame {
                    let tmp = source.deletingLastPathComponent()
                        .appendingPathComponent(".texttext-rename-\(UUID().uuidString)", isDirectory: true)
                    guard isCurrent(context) else { throw StalePass.invalidated }
