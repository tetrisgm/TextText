...
public final class EditorDocument {
    ...
    /// When our path no longer exists, follow the file by its textTextId so the
    ...
    private func relocateIfMoved() {
        ...
        let fileManager = FileManager.default
        guard !fileManager.fileExists(atPath: fileURL.path), let textTextId else { return }
        let match = WorkspaceIndexStore.identityFiles(root: workspaceRootURL)
            .first { $0.itemId == textTextId }
    ...
    private func adoptFrontMatter(_ adopted: Data) {
        ...
        textTextId = MarkdownIdentityCodec.extract(
    ...
    private func applyDiskData(_ data: Data) throws {
        ...
        title = split.frontMatter.flatMap(MarkdownFrontMatter.title(in:))
            ?? fileURL.deletingPathExtension().lastPathComponent
        textTextId = split.frontMatter.flatMap {
