...
public struct WorkspaceDocumentRecord: Equatable, Identifiable, Sendable {
    ...
    public var deepLink: URL {
        URL(string: "texttext-app://item/\(id.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? id)")!
    }
}
...
/// by the server post id (the same id the `texttext-app://item/{id}` deep link and
...
public struct WorkspaceIntentActions {
    ...
    public func openDocument(id: String) throws -> URL {
        ...
        guard !id.isEmpty,
              let url = URL(string: "texttext-app://item/\(id.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? id)") else {
