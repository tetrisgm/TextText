...
final class TextTextEnumeratorAdapter: NSObject, NSFileProviderEnumerator {
    ...
    init(
        container: TextTextItemIdentifier, core: WorkspaceEnumerator,
    ...
    func enumerateChanges(
        ...
        Task {
            ...
            case .success(let items):
                ...
                let delta = TextTextContainerDelta.compute(previous: previous, current: items)
                if !delta.updated.isEmpty {
                    observer.didUpdate(delta.updated.map(TextTextFileProviderItem.init))
                }
    ...
    static func bridge(_ error: TextTextSyncError) -> NSError {
        switch error {
        case .notFound:
