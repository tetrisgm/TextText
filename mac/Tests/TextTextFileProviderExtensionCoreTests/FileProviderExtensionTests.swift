...
final class FileProviderExtensionTests: XCTestCase {
    ...
    private func fileItem(
        ...
        let base = TextTextItemMapper.item(
            for: Fixtures.item(
                id: id, file: file, kind: "note",
                representation: representation),
            inFolder: folder, handle: "demo", readOnly: false)!
        return TextTextFileProviderItem(base.withFilename(file))
    }
    ...
    private func textBundle(
        ...
        try JSONEncoder().encode(TextTextTextBundleInfo()).write(
    ...
    ) throws -> TextTextFileProviderItem {
        let entry = Fixtures.item(
            id: id, file: filename, kind: "note", title: "My Note",
            representation: .textbundle)
        let item = try XCTUnwrap(TextTextItemMapper.item(
            for: entry, inFolder: "notes", handle: "demo", readOnly: false))
        return TextTextFileProviderItem(item.withFilename(filename))
    }

    private func version(_ hash: String) -> NSFileProviderItemVersion {
        let current = TextTextItemMapper.item(
            for: Fixtures.item(
                id: "p1", file: "a.md", kind: "note", title: "a", hash: hash),
            inFolder: "notes", handle: "demo", readOnly: false)!
        return NSFileProviderItemVersion(
            contentVersion: Data(hash.utf8),
            metadataVersion: TextTextFileProviderItem(current).itemVersion.metadataVersion)
    }

    private func version(_ item: TextTextItem) -> NSFileProviderItemVersion {
        TextTextFileProviderItem(item).itemVersion
    }

    private func version(
        contentHash: String, metadataFor item: TextTextItem
    ) -> NSFileProviderItemVersion {
        NSFileProviderItemVersion(
            contentVersion: Data(contentHash.utf8),
            metadataVersion: TextTextFileProviderItem(item).itemVersion.metadataVersion)
    }

    private func bookmarkWorkspace() -> TextTextWorkspace {
        let base = Fixtures.workspace()
        return TextTextWorkspace(
            blog: base.blog,
            folders: base.folders + [TextTextWorkspaceFolder(
                id: "bookmarks", name: "Bookmarks", path: "Bookmarks",
                mode: "bookmarks", parentId: nil)])
    }

    private func bookmarkEntry(hash: String = "h1") -> TextTextManifestItem {
        Fixtures.item(
    ...
    private var bookmarkCaptureURL: String {
        "https://texttext.public.blob.vercel-storage.com/captures/demo/b1/assets/hero.png"
    }

    private func documentArtifactManifest(
        hash: String = "h1"
    ) -> TextTextArtifactManifest {
        TextTextArtifactManifest(
    ...
    private func actionDefinitions() throws -> [[String: Any]] {
        ...
        let plistURL = macDirectory.appendingPathComponent(
            "Extensions/TextTextFileProviderExtension/Info.plist")
    ...
    func testCopyTextTextLinkUsesFreshAuthoritativeManifestURL() {
        ...
        api.manifests["notes"] = [Fixtures.item(
            ...
            url: "https://links.example/current-texttext-link")]
        var copied: [String] = []
        let provider = ext(api, copyLinkHandler: { copied.append($0); return true })
        let exp = expectation(description: "copy-texttext-link")
        var err: NSError?

        _ = provider.performAction(
            identifier: FileProviderExtension.copyTextTextLinkActionIdentifier,
        ...
        XCTAssertNil(err)
        XCTAssertEqual(copied, ["https://links.example/current-texttext-link"])
    }

    func testCopyTextTextLinkNeverExposesAuthenticatedContentEndpoint() {
        let api = FakeExtensionAPI(workspace: Fixtures.workspace())
        api.manifests["notes"] = [Fixtures.item(
            id: "p1", file: "a.md", kind: "note",
        ...
        let exp = expectation(description: "copy-relative-texttext-link")
    ...
    func testShareAndManageAccessOpenTextTextAppDeepLinks() {
        let api = FakeExtensionAPI(workspace: Fixtures.workspace())
        api.manifests["notes"] = [Fixtures.item(
            id: "p1", file: "a.md", kind: "note",
            url: "/api/sync/v1/files/p1",
            canonicalUrl: "https://links.example/current-texttext-link")]
        var opened: [URL] = []
        ...
        XCTAssertEqual(opened.map(\.scheme), ["texttext-app", "texttext-app"])
        ...
        XCTAssertEqual(opened.compactMap {
            ...
            "https://links.example/current-texttext-link",
            "https://links.example/current-texttext-link",
        ])
    }
    ...
    func testFetchContentsRetriesUntilMetadataAndBytesShareOneRevision() throws {
        ...
        api.fileTextResults = [
            .success(TextTextFileContent(text: "old bytes", hash: "h1")),
            .success(TextTextFileContent(text: "new bytes", hash: "h2")),
        ]
        ...
        XCTAssertEqual(
            ...
            TextTextFileProviderItem.nativeMaterializationVersion + "markdown:h2")
    }
    ...
    func testBookmarkFetchMaterializesOneTextBundleWithCapturedAssets() throws {
        ...
        api.fileTextResults = [
            .success(TextTextFileContent(text: canonical, hash: "h1")),
        ]
        api.artifactManifests["b1"] = documentArtifactManifest()
        let png = Data([0x89, 0x50, 0x4E, 0x47])
        api.artifactContents[bookmarkCaptureURL] = TextTextArtifactContent(
        ...
        XCTAssertTrue(
            contentType == .package
                || contentType.identifier == TextTextItem.textBundleTypeIdentifier)
    ...
    func testTextpackFetchReturnsLeafWithExactArchiveSize() throws {
        ...
        api.fileTextResults = [
            .success(TextTextFileContent(text: "# A", hash: "h1")),
        ]
        let directory = tempDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
    ...
    func testReimportAdoptsWorkspaceByParentAndFilename() {
        ...
        let template = ReimportTemplateItem(
            ...
            parent: TextTextItemIdentifier.rootContainer.rawValue,
    ...
    func testReimportAdoptsRawReservedAndCanonicallyEquivalentTextFilename() {
        ...
        api.manifests["notes"] = [Fixtures.item(
            id: "p1",
            file: TextTextFilename.filename(
    ...
    func testCreateVersionConvergesWithEnumerationForCollidingTextPack() async throws {
        ...
        let enumeratedVersion = TextTextFileProviderItem(serverItem).itemVersion
    ...
    func testCreateTextBundleUploadsAssetsBeforeCommittingMarkdown() throws {
        ...
        let uploadedURL =
            "https://texttext.public.blob.vercel-storage.com/documents/demo/n9/assets/photo.png"
        api.uploadAssetResult = .success(TextTextArtifact(
    ...
    func testCreateFolderCallsCreateFolderWithParentPath() {
        let api = FakeExtensionAPI(workspace: Fixtures.workspace())
        let folder = TextTextWorkspaceFolder(id: "tmp", name: "Ideas", path: "Blog/Ideas", mode: "blog", parentId: "blog")
        let template = TextTextFileProviderItem(TextTextItemMapper.item(for: folder, handle: "demo", readOnly: false))
    ...
    func testCreateFolderDecodesPortableFinderComponent() {
        ...
        let template = TextTextFileProviderItem(
            TextTextItemMapper.item(for: folder, handle: "demo", readOnly: false))
    ...
    func testTextBundleEditKeepsUnchangedAssetURLWithoutUploadingAgain() throws {
        ...
        _ = ext(api, temporaryDirectory: directory).modifyItem(
            TextTextFileProviderItem(item), baseVersion: version(item),
    ...
    func testTextBundleEditUploadsNewAssetBeforeMarkdownRevision() throws {
        ...
        api.manifests["bookmarks"] = [entry]
        let uploadedURL =
            "https://texttext.public.blob.vercel-storage.com/documents/demo/b1/assets/new.png"
        api.uploadAssetResult = .success(TextTextArtifact(
    ...
    func testInvalidDestinationParentFailsBeforeCompoundUpload() {
        ...
        let base = TextTextItemMapper.item(
            ...
            inFolder: "notes", handle: "demo", readOnly: false)!
        let item = TextTextFileProviderItem(
    ...
    func testCompoundModifyPatchesWithPutReturnedHash() {
        ...
        let api = FakeExtensionAPI(workspace: Fixtures.workspace())
        api.putResult = .success(TextTextManifestItem(
    ...
    func testCompoundModifyReturnsMetadataPendingAfterContentSave() throws {
        ...
        XCTAssertEqual(
            ...
            TextTextFileProviderItem.nativeMaterializationVersion + "markdown:puthash")
        XCTAssertEqual(api.putCalls.count, 1)
        XCTAssertEqual(api.patchCalls.count, 1)
    }
    ...
    func testRenameFileDecodesPortableNameAndRemovesItsCollisionSuffix() {
        let api = FakeExtensionAPI(workspace: Fixtures.workspace())
        let suffix = TextTextFilename.collisionSuffix("p1")
    ...
    func testRenameConflictReturnsCurrentRemoteItemWithoutReplayingIntent() {
        ...
        let remote = TextTextItemMapper.item(
    ...
    func testStaleFileMetadataBaseReturnsCurrentItemWithoutPatch() {
        let api = FakeExtensionAPI(workspace: Fixtures.workspace())
        let original = TextTextItemMapper.item(
            for: Fixtures.item(
    ...
    func testBoundedCanonicalFilenameDoesNotRetitleStoredFullTitle() {
        ...
        XCTAssertLessThanOrEqual(
            current.filename.utf8.count, TextTextFilename.maximumComponentUTF8Length)
        let exp = expectation(description: "bounded-no-op")
        var result: NSFileProviderItem?

        _ = ext(api).modifyItem(
            TextTextFileProviderItem(current), baseVersion: version(current),
    ...
    func testRenameFolderCallsRenameFolder() {
        ...
        let current = TextTextItemMapper.item(for: folder, handle: "demo", readOnly: false)
        let item = TextTextFileProviderItem(
            current.withFilename(TextTextFilename.encodeComponent("Writing")))
    ...
    func testRenameFolderAndWorkspaceDecodePortableNames() {
        let api = FakeExtensionAPI(workspace: Fixtures.workspace())
        let folder = api.workspaceValue.folders.first { $0.id == "blog" }!
        let currentFolder = TextTextItemMapper.item(
            for: folder, handle: "demo", readOnly: false)
        let folderItem = TextTextFileProviderItem(
            currentFolder.withFilename(TextTextFilename.encodeComponent("Why??")))
        ...
        let currentWorkspace = TextTextItemMapper.workspaceItem(
            handle: "demo", name: "Demo", readOnly: false)
        let workspaceItem = TextTextFileProviderItem(
            currentWorkspace.withFilename(
                "What~3F~3F\(TextTextFilename.collisionSuffix("demo"))"))
    ...
    func testRenameWorkspaceCallsRenameWorkspace() {
        let api = FakeExtensionAPI(workspace: Fixtures.workspace())
        let current = TextTextItemMapper.workspaceItem(
            handle: "demo", name: "Demo", readOnly: false)
        let item = TextTextFileProviderItem(
            current.withFilename(TextTextFilename.encodeComponent("Studio")))
    ...
    func testWorkspaceRenameAndModificationDateStillRenamesWorkspace() {
        ...
        let current = TextTextItemMapper.workspaceItem(
            handle: "demo", name: "Demo", readOnly: false)
        let local = TextTextFileProviderItem(
            current.withFilename(TextTextFilename.encodeComponent("Studio")))
    ...
    func testStaleMountedFolderNameDoesNotOverwriteRemoteRename() {
        ...
        let api = FakeExtensionAPI(workspace: TextTextWorkspace(
            blog: original.blog, folders: renamedFolders))
        let old = TextTextItemMapper.item(for: oldFolder, handle: "demo", readOnly: false)
        let local = TextTextFileProviderItem(
            old.withFilename(TextTextFilename.encodeComponent("Local Folder")))
    ...
    func testStaleMountedWorkspaceNameDoesNotOverwriteRemoteRename() {
        ...
        let old = TextTextItemMapper.workspaceItem(
            handle: "demo", name: "Demo", readOnly: false)
        let local = TextTextFileProviderItem(
            old.withFilename(TextTextFilename.encodeComponent("Local Workspace")))
    ...
    func testBeforeFirstSyncMetadataSentinelCannotRenameFolder() {
        ...
        let current = TextTextItemMapper.item(for: folder, handle: "demo", readOnly: false)
        let local = TextTextFileProviderItem(
            current.withFilename(TextTextFilename.encodeComponent("Local Folder")))
        let beforeFirst = NSFileProviderItemVersion(
