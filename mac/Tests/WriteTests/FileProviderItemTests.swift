...
final class FileProviderItemRegressionTests: XCTestCase {
    ...
    private func item(
        ...
        try XCTUnwrap(WriteItemMapper.item(
            for: manifest(
    ...
    func testTextPackLeafAdvertisesSizeWhileTextBundleDirectoryDoesNot() throws {
        ...
        let textPack = WriteFileProviderItem(try item(
            id: "pack", title: "Packed", representation: .textpack,
            size: advertisedSize))
        let textBundle = WriteFileProviderItem(try item(
    ...
    func testRenameChangesDisplayMetadataWithoutChangingStableIdentity() throws {
        ...
        let originalProviderItem = WriteFileProviderItem(original)
        let renamedProviderItem = WriteFileProviderItem(renamed)
    ...
    func testDuplicateTitlesDisambiguateStablyWithoutCollisions() throws {
        ...
        let forward = WriteFilename.disambiguate([first, second])
        let reversed = WriteFilename.disambiguate([second, first])
