...
final class TextTextItemMapperTests: XCTestCase {
    private let h = "demo"

    func testTopLevelFolderParentsToWorkspace() {
        let f = Fixtures.folder("blog", "Blog")
        let item = TextTextItemMapper.item(for: f, handle: h, readOnly: true)
        XCTAssertEqual(item.parentIdentifier, .workspace(h))
        XCTAssertEqual(item.identifier, .folder(handle: h, id: "blog"))
        XCTAssertEqual(item.typeIdentifier, TextTextItem.folderTypeIdentifier)
    }
    ...
    func testWorkspaceItemIsAContainerNamedForTheWorkspace() {
        let item = TextTextItemMapper.workspaceItem(handle: h, name: "Shoku's Space", readOnly: false)
    ...
    func testWorkspaceItemFallsBackToHandleWhenNameEmpty() {
        let item = TextTextItemMapper.workspaceItem(handle: h, name: "", readOnly: false)
        XCTAssertEqual(item.filename, h)
    }

    func testWorkspaceAndFolderNamesUsePortableReversibleComponents() {
        let workspace = TextTextItemMapper.workspaceItem(
            handle: h, name: ".Research?? ", readOnly: false)
        XCTAssertEqual(TextTextFilename.decodeComponent(workspace.filename), ".Research?? ")
        XCTAssertFalse(workspace.filename.contains("?"))
        XCTAssertFalse(workspace.filename.hasPrefix("."))

        let folder = TextTextItemMapper.item(
            for: Fixtures.folder("f", "A/B: C"), handle: h, readOnly: false)
        XCTAssertEqual(TextTextFilename.decodeComponent(folder.filename), "A/B: C")
        XCTAssertFalse(folder.filename.contains("/"))
        XCTAssertFalse(folder.filename.contains(":"))
    }

    func testKindMapping() {
        let cases: [(String, TextTextItemKind)] = [
        ...
        for (raw, expected) in cases {
            ...
            let item = TextTextItemMapper.item(for: entry, inFolder: "blog", handle: h, readOnly: true)
            XCTAssertEqual(item?.kind, expected)
            XCTAssertEqual(item?.typeIdentifier, TextTextItem.markdownTypeIdentifier)
    ...
    func testRepresentationExplicitlyControlsFilenameAndTypeIdentifier() throws {
        let cases: [(TextTextFileRepresentation, String, String)] = [
            (.textbundle, "Title.textbundle", TextTextItem.textBundleTypeIdentifier),
            (.markdown, "Title.md", TextTextItem.markdownTypeIdentifier),
            (.text, "Title.txt", TextTextItem.plainTextTypeIdentifier),
        ]
    ...
    func testEntryWithoutIdIsSkipped() {
        ...
        XCTAssertNil(TextTextItemMapper.item(for: entry, inFolder: "notes", handle: h, readOnly: true))
    }
    ...
    func testTitlelessDraftFallsBackToSlug() {
        let entry = TextTextManifestItem(
            ...
            date: nil, createdAt: nil, updatedAt: nil, url: nil)
        let item = TextTextItemMapper.item(for: entry, inFolder: "notes", handle: h, readOnly: true)
    ...
    func testQuestionMarksInTitleProduceSafeReversibleFilename() {
        ...
        XCTAssertEqual(TextTextFilename.titleFromFilename(item?.filename ?? ""), "Why??")
    }

    func testHashCarriesForConflictChecks() {
        let entry = Fixtures.entry(id: "p1", file: "a.md", kind: "article", title: "A", hash: "deadbeef")
        let item = TextTextItemMapper.item(for: entry, inFolder: "blog", handle: h, readOnly: false)
    ...
    func testDocumentSizeComesFromManifestSize() {
        ...
        let entry = TextTextManifestItem(
            ...
            size: 4096)
        let item = TextTextItemMapper.item(for: entry, inFolder: "notes", handle: h, readOnly: false)
    ...
    func testWithContentCarriesHashAndSize() {
        ...
        let base = TextTextItemMapper.item(for: entry, inFolder: "notes", handle: h, readOnly: false)!
