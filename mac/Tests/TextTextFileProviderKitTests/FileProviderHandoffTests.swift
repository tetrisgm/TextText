...
final class FileProviderHandoffTests: XCTestCase {
    ...
    func testStoreAccessGroupReadsEnvOverride() {
        setenv("TEXTTEXT_KEYCHAIN_GROUP", "TEAMID123.app.texttext.fp", 1)
        defer { unsetenv("TEXTTEXT_KEYCHAIN_GROUP") }
        XCTAssertEqual(FileProviderHandoffStore.accessGroup(), "TEAMID123.app.texttext.fp")
    }

    func testStoreSaveFailsWithoutAccessGroup() {
        unsetenv("TEXTTEXT_KEYCHAIN_GROUP")
