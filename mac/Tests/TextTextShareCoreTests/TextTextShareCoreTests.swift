...
final class TextTextShareCoreTests: XCTestCase {
    ...
    func testPrepareRendersEachKindForServerCreation() throws {
        ...
        XCTAssertFalse(noteBody.contains("textTextId"), "the server assigns the id; none is injected")
        ...
        XCTAssertFalse(bookmarkBody.contains("textTextId"))
    ...
    func testPrepareAppendCarriesTargetAndText() throws {
        let filer = InboxFiler(now: fixedDate)
        guard case let .append(targetTextTextId, text) =
            try filer.prepare(record(InboxItem(
                kind: .append, text: "Appended body", targetTextTextId: "target-id")))
        else { return XCTFail("append should prepare as .append") }
        XCTAssertEqual(targetTextTextId, "target-id")
    ...
    func testShareFactoryCreatesBookmarkAndAppendItems() throws {
        ...
        let append = try ShareInboxItemFactory.makeItem(
            ...
            targetTextTextId: "texttext-123"
        )
        XCTAssertEqual(append.kind, .append)
        XCTAssertEqual(append.targetTextTextId, "texttext-123")
    }
    ...
    func testMarkdownPreviewRendersConstructsAndHidesFrontMatter() throws {
        ...
        XCTAssertFalse(html.contains("textTextId"))
        XCTAssertFalse(html.contains("textTextFolderId"))
        XCTAssertFalse(html.contains("textTextKind"))
        XCTAssertFalse(html.contains("texttext.markdown-file.v1"))
    }
    ...
    func testMarkdownPreviewEscapesHTMLAndDropsRemoteResources() throws {
        ...
        let html = TextTextMarkdownPreviewRenderer.renderHTML(markdown: markdown, workspaceRootURL: try temporaryDirectory())
    ...
    private func record(_ item: InboxItem, id: String = "rec-1") -> InboxRecord {
        InboxRecord(
            id: id, item: item,
            directoryURL: URL(fileURLWithPath: "/tmp/texttext-share-\(id)", isDirectory: true),
            payloadURL: nil)
    }
    ...
    private func temporaryDirectory() throws -> URL {
        let url = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("TextTextShareCoreTests-\(UUID().uuidString)", isDirectory: true)
