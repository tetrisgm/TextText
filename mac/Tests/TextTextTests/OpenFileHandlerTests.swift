...
final class OpenFileHandlerTests: XCTestCase {
    func testLegacyItemLinkPreservesExistingOpenBehavior() {
        let link = TextTextItemLink(
            url: URL(string: "texttext-app://item/item-123")!
        )
    ...
    func testAgentItemLinkTargetsExactWorkspaceAndEditMode() {
        let link = TextTextItemLink(
            url: URL(
                string: "texttext-app://item/item-123?workspace=shoku-space&mode=edit"
            )!
        )
    ...
    func testItemLinkRejectsUnsafeOrAmbiguousTargets() {
        XCTAssertNil(TextTextItemLink(
            url: URL(string: "texttext-app://item/item%2Fother?workspace=shoku-space")!
        ))
        XCTAssertNil(TextTextItemLink(
            url: URL(string: "texttext-app://item/item-123?workspace=shoku%2Fspace")!
        ))
        XCTAssertNil(TextTextItemLink(
            url: URL(
                string: "texttext-app://item/item-123?workspace=one&workspace=two"
            )!
        ))
        XCTAssertNil(TextTextItemLink(
            url: URL(string: "texttext-app://item/item-123?mode=compose")!
        ))
    }

    func testExplicitModeOverridesTheDefaultKindBehavior() {
        let note = TextTextItemOpenTarget(
            handle: "workspace",
            itemId: "note-id",
            slug: "note",
            kind: "note"
        )
        let article = TextTextItemOpenTarget(
            handle: "workspace",
            itemId: "article-id",
            slug: "article",
            kind: "article"
    ...
    func testPendingItemLinksKeepTheLatestModeForOneExactTarget() {
        ...
        pending.enqueue(TextTextItemLink(
            url: URL(
                string: "texttext-app://item/item-123?workspace=shoku-space&mode=read"
            )!
        )!)
        pending.enqueue(TextTextItemLink(
            url: URL(
    ...
    func testPendingItemLinksStayBoundedDuringColdLaunch() {
        ...
        for index in 0..<12 {
            pending.enqueue(TextTextItemLink(
                url: URL(
                    string: "texttext-app://item/item-\(index)?workspace=shoku-space"
                )!
            )!)
        }
    ...
    func testOnlyTextTextFileProviderFileIdentifiersAreManaged() {
        XCTAssertTrue(OpenFileHandler.isTextTextFileProviderItem("file:workspace:item-id"))
        XCTAssertFalse(OpenFileHandler.isTextTextFileProviderItem("folder:workspace:folder-id"))
        XCTAssertFalse(OpenFileHandler.isTextTextFileProviderItem("unrelated-provider-id"))
        XCTAssertFalse(OpenFileHandler.isTextTextFileProviderItem(nil))
    }
    ...
    func testTextPackWorkspaceFileOpensAsManagedTarget() throws {
        ...
        """
        let bundle = try TextTextTextBundlePackage.materialize(
            canonicalMarkdown: markdown, assets: [], sourceURL: nil, in: root)
        let zipped = try TextTextTextBundlePackage.zipToTextPack(
