...
final class SyncEngineEpochTests: XCTestCase {
    ...
    func testLegacyRootInsideTextTextFileProviderMountIsRejectedBeforeUse() throws {
        ...
        client.workspaceValue = workspace()
        let engine = makeEngine(store: store, root: LockedValue(root), client: client)
        engine.isInsideTextTextFileProviderMount = { $0.standardizedFileURL == root.standardizedFileURL }
    ...
    private func temporaryDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("TextTextEpochTests-\(UUID().uuidString)", isDirectory: true)
