...
final class SyncEngineRegressionTests: XCTestCase {
    func testFreshInstallIgnoresForeignRootIndexAndDoesNotDeleteMissingFiles() throws {
        ...
        try FileManager.default.createDirectory(at: root.appendingPathComponent(".texttext/state"), withIntermediateDirectories: true)
        ...
        try Data("foreign marker".utf8).write(to: root.appendingPathComponent(".texttext/state/sync-marker.txt"))
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent(".texttext-local.nosync/state/sync-marker.txt").path))
    }
    ...
    func testCopyWithSameTextTextIdIsNotPostedAsDuplicateNorRenamesOriginal() throws {
    ...
    func testPausedEngineWritesNothingAndSettlesIdle() throws {
        ...
        // no-op pass, create no ~/TextText directory, make no client call, and
    ...
    func testCorruptRootIndexDoesNotBaselineAndOverwriteUnsyncedEdit() throws {
        ...
        try write("{", to: root.appendingPathComponent(".texttext/index.json"))
    ...
    func testMassMissingFilesPauseServerDeletes() throws {
        ...
        fake.workspaceValue = fixtureWorkspace()
        fake.manifestReplies["notes"] = .notModified
        try write("marker\nmirror-id: era-m\n", to: root.appendingPathComponent(".texttext-local.nosync/state/sync-marker.txt"))
    ...
    func testMoveWithLocalContentEditPushesBeforeCanonicalizing() throws {
        ...
        try write(
            ...
            to: root.appendingPathComponent(
                ".texttext-local.nosync/state/sync-marker.txt"))
    ...
    private func writeLocalMarker(root: URL) throws {
        try write("local marker", to: root.appendingPathComponent(".texttext-local.nosync/state/sync-marker.txt"))
    }

    private func temporaryDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("TextTextTests-\(UUID().uuidString)", isDirectory: true)
