...
final class FakeExtensionAPI: WriteSyncAPI, @unchecked Sendable {
    ...
    struct CreateFileCall: Equatable { let body: String; let folderId: String? }
    struct PatchCall: Equatable { let postId: String; let folderId: String?; let slug: String? }
    struct PutCall: Equatable { let postId: String; let body: String; let hash: String }
    struct CreateFolderCall: Equatable { let parentPath: String; let name: String }
    ...
    func createFile(body: String, folderId: String?) async -> Result<WriteManifestItem, WriteSyncError> {
        createFileCalls.append(CreateFileCall(body: body, folderId: folderId))
    ...
    func patchFile(postId: String, folderId: String?, slug: String?) async -> Result<WriteManifestItem, WriteSyncError> {
        patchCalls.append(PatchCall(postId: postId, folderId: folderId, slug: slug))
    ...
    func deleteFile(postId: String) async -> Result<Void, WriteSyncError> {
        deleteCalls.append(postId); return deleteResult
    }
    func createFolder(parentPath: String, name: String) async -> Result<WriteWorkspaceFolder, WriteSyncError> {
        createFolderCalls.append(CreateFolderCall(parentPath: parentPath, name: name))
