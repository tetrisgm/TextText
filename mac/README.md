...
Run the web app, then the client against it:
    ...
    WRITE_SERVER=http://localhost:3000 WRITE_DEV_NO_MOVE=1 \
      swift run --package-path mac

- `WRITE_SERVER` points the client at any server origin (dev override).
- `WRITE_DEV_NO_MOVE=1` skips the move-to-Applications prompt. It is an env
  var on purpose, never a persisted default.
- `WRITE_STATE_DIR` relocates the state dir (credentials, index, trash) away
  from `~/Library/Application Support/Write` for experiments.
...
One real sync pass through the real engine, one JSON line, exit 0/1:
    ...
    WRITE_HEADLESS=1 \
    WRITE_TOKEN=wsk_... \
    WRITE_SERVER=http://localhost:3000 \
    WRITE_SYNC_ROOT=/tmp/write-sync \
    WRITE_STATE_DIR=/tmp/write-state \
      swift run --package-path mac
...
1. Developer ID Application certificate in the login keychain (the build
   script auto-detects it; `WRITE_SIGN_ID` overrides).
2. Notary profile: create an app-specific password at appleid.apple.com,
   then
   `xcrun notarytool store-credentials "write-notary" --apple-id "<apple-id-email>" --team-id "<TEAMID>"`.
3. Sparkle keys, ONCE: `swift build --package-path mac` materializes the
   ...
   public key in your release env (`WRITE_SPARKLE_PUBLIC_KEY`).
