...
Home, Headlines and Profile are the three primary destinations, represented
by the house, globe and person in a bottom bar. A single news column is
...
Later, Reading History, Publisher Subscriptions, the library and settings.
...
17px semibold with approximately 1.3 line height. Publisher labels are 13px;
metadata is 12px. Topic tabs use the headline size. Selection is ink and
weight, with no pill or underline.

Light uses white with near-black text. Dark uses `#0b0b0c` with `#f2f2f2`
...
Contextual actions stay behind the story menu. Feedback leaves a dimmed row
